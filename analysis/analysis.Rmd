...
d.fc <- function(response, cond) {
  ...
  # calculate 95 CI (parametrically)
  ...
  es <- data.frame(effect_size = effect_size, cill = cill, 
                   ciul = ciul, var_d = var_d)
...
# Note: The proportions described in the original report were obtained by aggregating across *trials* not participants (2 trials/participant). We correct for this here by taking the proportion of participants, which results in slightly different proportions than those reported in the original report.
...
<a name="t4"/>
<h3> Experiment 5 - Turk Replication #5 </h3>
...
# make factors
t4$Answer.sample <- factor(t4$Answer.sample, labels=c('teacher','learner')) # sample0 = teacher, sample1=learner
...
if (proper){
  proper <- (t4.f$Answer.Qproper1 == 1 & t4.f$Answer.Qproper2 == 1 & 
            t4.f$Answer.Qproper3 == 1) & t4.f$Answer.Qsub1 == 0 &
            t4.f$Answer.Qsub2 == 0 & t4.f$Answer.Qbasic1 == 0 &
            t4.f$Answer.Qbasic2 == 0 & t4.f$Answer.Qbasic3 == 0 
  
  t4.f$w.ans.cat[proper] <- "proper"
  t4.f$w.ans.cat = factor(t4.f$w.ans.cat, levels=c("proper", "sub", "basic", "other"))
} else { 
...
# get proportions and bootstrapped CIS
M <- data.frame()
for (k in 1:length(levels(t4.f$w.ans.cat))){
    M = rbind(M, ddply(t4.f, .(Answer.sample), 
                     function (d) {get.prop.CIs(d$w.ans.cat, levels(d$w.ans.cat)[k])}))
}
...
```{r,echo = F}
ggplot(t4.f.props, aes(x = factor(w.ans.cat), y = prop, fill = Answer.sample)) +
  geom_bar(stat = "identity", position=position_dodge()) +
  geom_linerange(aes(ymin = ciwl,ymax = ciul), position = position_dodge(.9)) +
  ylim(0,1) +
  ylab("Prop. participants") +
...
lf.props = rbind(t1.f.props, t2.f.props, t3.f.props, t4.f.props, l1.f.props)
...
all.props$exp <- ordered(as.factor(all.props$exp), 
                    levels = c( "X&T (2007b) children", "X&T (2007b)", "Turk #1", 
                                "Turk #2", "In Lab", "Turk #3",  "Turk #4" ))

all.props$exp = revalue(all.props$exp, c("X&T (2007b)" = "X&T adults", "X&T (2007b) children" = "X&T children", "Turk #1" = "Exp. 1", "Turk #2" = "Exp. 2", "In Lab" = "Exp. 3", "Turk #3" = "Exp. 4", "Turk #4" = "Exp. 5"))
...
ggplot(all.props.crit, aes(x = w.ans.cat, y = prop, 
                           fill = Answer.sample)) +
  geom_bar(stat="identity", position = position_dodge()) +
  facet_grid(. ~ exp) +
  geom_rect(data = all.props.crit[21:28,], fill = "gray", 
...
t4.f.es = d.fc(t4.f$w.ans.cat, t4.f$Answer.sample)
...
all.es = rbind(t1.f.es, t2.f.es, t3.f.es, t4.f.es, l1.f.es, xt.a.es, xt.c.es)
all.es$exp = c("Turk #1" ,"Turk #2", "Turk #3", "Turk #4", "In Lab",
               "X&T adults", "X&T children")
all.es$exp = revalue(all.es$exp, c( "Turk #1" = "Exp. 1",
                                "Turk #2" = "Exp. 2", "In Lab" = "Exp. 3", 
                                "Turk #3" = "Exp. 4", "Turk #4" = "Exp. 5"))
names(all.es)[1] = "effect_size_d"

# merge together all raw data so can also get log odds ratio
...
all.data.f.counts = all.data.f %>%
                    group_by(exp, Answer.sample, w.ans.cat) %>%
                    summarise(counts = n())
all.data.f.counts.short = dcast(all.data.f.counts, value = counts, 
                                exp ~ Answer.sample + w.ans.cat, mean)
all.data.f.counts.short$n = rowSums(all.data.f.counts.short[,2:5])

# get log odds ratio
all.lor = escalc(measure = "OR", ai = learner_basic, bi = learner_sub, 
       ci = teacher_basic, di = teacher_sub, 
       data = all.data.f.counts.short, append = TRUE) # this is uncorrected log odds ratio
names(all.lor)[7] = "effect_size_lor"
names(all.lor)[8] = "var_lor"

all.es = left_join(all.es, all.lor, by="exp")

all.es$exp <- ordered(as.factor(all.es$exp ), 
                    levels = c( "X&T children", "X&T adults", "Exp. 1", 
                                "Exp. 2", "Exp. 3", "Exp. 4",  "Exp. 5" ))
all.es = all.es[c(7, 6, 1,2,5, 3, 4),]

## fixed effects
# cohen's d
fixed.effects.d = rma(effect_size_d, var_d, data = all.es, method = "FE")
forest(fixed.effects.d, slab=all.es$exp,   mlab = "FE estimate in d")

# lor
fixed.effects.lor = rma(effect_size_lor, var_lor, data = all.es, method = "FE")
forest(fixed.effects.lor, slab=all.es$exp,mlab = "FE estimate in log odds")

## random effects
# cohen's d
random.effects.d = rma(effect_size_d, var_d, data = all.es)
forest(random.effects.d, slab=all.es$exp,   mlab = "RE estimate in d")

# lor
random.effects.lor = rma(effect_size_lor, var_lor, data = all.es)
forest(random.effects.lor, slab=all.es$exp, mlab = "RE estimate in log odds")
```
...
<a name="power5"/>
<h3> Power analysis, based on effect size estimated from Exps 1-4 and XT-Adult </h3>
```{r}
# fixed effects
fixed.effects = rma(effect_size_d, var_d, data = all.es[2:6,], method = "FE")
...
# random effects
random.effects = rma(effect_size_d , var_d, data = all.es[2:6,])#
...
```{r, include = F, eval = F}
# Mikes Forrest
  f <- fitted(rma(effect_size, var_d, data = all.es))
  p <- predict(rma(effect_size, var_d, data = all.es))
   ...
   forest_data <- data.frame(effects = as.numeric(model()$yi.f),
    ...
    qplot(exp, effect_size, ymin = cill, ymax = ciul,
          geom = "linerange",
          data = all.es) +
      #geom_point(aes(y = effect_size, size = n)) +
      geom_point(aes(y = effect_size)) +
      #geom_pointrange(aes(x = exp,
       #             y = effect_size,
        #            ymin = cill,
        #            ymax = ciul),
        #        col = "red") +
...
# get effect size
all.es.mean = d.fc(all.data.f$w.ans.cat, all.data.f$Answer.sample)
print(paste("Effect size:", all.es.mean[1]))

power.analysis = pwr.chisq.test(w = all.es.mean[1],df = 1, sig.level = 0.05, power = .95)

power.analysis 

print(paste("N for .95 power (raw average):", power.analysis$N))

#pdf("../../writeup/figures/FIG_3.pdf", height = 4, width = 4)
ggplot(all.es, aes(x = exp, y = effect_size_d, ymin = cill, ymax = ciul)) + 
