...
all.data.f.props$response.cat  = revalue(all.data.f.props$response.cat, c("sub" = "sub."))
...
all.data.f.props %>%
  ...
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_grid(. ~ exp) +
  geom_rect(data = all.data.f.props[21:28,], fill = "gray", 
...
all.es = propes(all.data.f.props.basic$learner_prop, 
           ...
           all.data.f.props.basic$teacher_n, dig=3,
           verbose = F)
...
# fixed effects (metafor package)
fixed.effects.r = rma(r, var.r, data = all.es, method = "FE")
fixed.effects.r

# random effects
random.effects.r = rma(r, var.r, data = all.es)
random.effects.r
```
...
<h4> based on Exp 1-4 and X&T  </h4>
...
w1 =  sqrt(chisq.test(t3_tab, correct = F)$statistic/sum(t3_tab))
w1

## METHOD 2 - this only works in the univarate case
# this is different from w1 above because it's bivariate
w2 =  sqrt(sum(((prop.table(t3_tab, 2)-.5)^2)/.5))
w2

# but, if you just take the first row of t3 and normalize correctly, w1 = w2
# w1
top_row = t3_tab[,1]
top_row
sqrt(chisq.test(top_row)$statistic/sum(top_row))

# w2
sqrt(sum(((prop.table(top_row)-.5)^2)/.5))

# According to Olivier and Bell (2013):
# (1) cohen's w is called phi in the literature
# (2) phi = pearson's r for the 2X2 case

# so, try to use r from es estimates above (all.es)

# BUT: 
# r from compute.es package != w1 above. 
all.es[all.es$exp == "Exp. 4", "r"]
w1

# w1 != r for adult xt data either...
xt_a_tab = table(xt_data.adults$w.ans.cat, xt_data.adults$Answer.sample)
sqrt(chisq.test(xt_a_tab)$statistic/sum(xt_a_tab))
all.es[all.es$exp == "X&T adults", "r"]

# But, if we calculate r directly from the equation in O&B r_ob = w1
calculate_prop_r <- function(response, condition) {
  tb = prop.table(table(response, condition))
  p_11 = tb[4]
  p_1m = tb[4] + tb[2]
  p_m1 = tb[4] + tb[3]
  num = p_11-(p_1m * p_m1)
  denom = sqrt((p_1m * p_m1)*(1-p_1m)*(1-p_m1)) 
  r = num/denom
  return (r)
}
...
all.es = all.data.f %>%
  group_by(exp) %>%
  summarise(r_ob = calculate_prop_r(w.ans.cat, Answer.sample)) %>%
  left_join(all.es)

all.es[all.es$exp == "Exp. 4", "r_ob"]
w1

# Now we can almost use rma to use metanalytic methods to calculate effect size, but we don't know variances on r....

# But, r and r_ob are almost perfectly correlated
cor(all.es$r, all.es$r_ob)
intercept = lm(all.es$r ~ all.es$r_ob)$coefficients[1]
intercept
all.es$r_ob + intercept
all.es$r

# what's the difference between how r and r_ob are calculated?
# from compute.es package (r: or-> lor -> d -> r)
# from ob (r_ob: p -> r)

# covert r_ob to d, but this is not the same as compute.es d
all.es$d_ob = 2*all.es$r_ob/sqrt(1-all.es$r_ob^2)
