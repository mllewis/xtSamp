...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**[(9) Power Analysis for Exp. 6](#power6)** <br/>
...
opts_chunk$set(echo = T, message = T, warning = T, error = T, cache = F)
```
...
<a name = "power6"/>
<h3> Power analysis for Exp. 6 </h3>
<h4> ES based on Exp 1-5  </h4>

```{r exp6_power,include = F}
# fixed effects
fixed.effects.turk.badStim = rma(r, var.r, data = all.es[3:4,],
                                 method = "FE")$b # exp 1 and 2
fixed.effects.lab.badStim = all.es[5,"r"] # exp 3
fixed.effects.turk.goodStim = rma(r, c(.01, .01),
                                  data = all.es[6:7,], method = "FE")$b # exp 4 and 5
#fixed.effects.turk.badStim 
#fixed.effects.lab.badStim
#fixed.effects.turk.goodStim 
stimEffect = fixed.effects.turk.goodStim - fixed.effects.turk.badStim
personEffect = fixed.effects.lab.badStim - fixed.effects.turk.badStim 
exp6_es = fixed.effects.turk.badStim + stimEffect + personEffect
power.analysis = pwr.chisq.test(w = exp6_es,
                                df = 1, 
                                sig.level = 0.05,
                                power = .95)
power.analysis
print(paste("N for .95 power (fixed effects):",
            power.analysis$N))
print(paste("Extra: ", .25 * power.analysis$N))

# random effects
random.effects.turk.badStim = rma(r, var.r, data = all.es[3:4,])$b
random.effects.lab.badStim = all.es[5,"r"]
random.effects.turk.goodStim = rma(r, c(.01, .01),
                                  data = all.es[6:7,])$b

stimEffect = random.effects.turk.goodStim - random.effects.turk.badStim
personEffect = random.effects.lab.badStim - random.effects.turk.badStim 
exp6_es = random.effects.turk.badStim + stimEffect + personEffect
...
print(paste("N for .95 power (fixed effects):",
            power.analysis$N))
print(paste("Extra: ", .25 * power.analysis$N))
#Using the most conservative estimate, we would need 74 participants. Assuming about 25% data loss as in Experiment 3, this would mean we would need to run 19 extra participants, for an approximate total of N = 100.
```
...
# all
all.es.fixed <- all.es
all.es.fixed[7, "var.r"] = .01
all.es.estimate = rma(r, var.r, data = all.es.fixed,
                                 method = "FE")$b
power.analysis = pwr.chisq.test(w = all.es.estimate,
                                df = 1, 
                                sig.level = 0.05,
                                power = .95)
power.analysis
...
fixed.effects.turk.goodStim = rma(r, c(.01, .01),
                                  data = all.es[6:7,], method = "FE")$b # exp 4 and 5
fixed.effects.lab.badStim = all.es[5,"r"]
personEffect = fixed.effects.lab.badStim - fixed.effects.turk.badStim 
exp6_es = fixed.effects.lab.badStim + personEffect

# convert es E6 to proportions (I got these from just messing around with propes function using exp6_es) and get in tabular form
learner_prop_basic = .28
teacher_prop_basic = .1
learner_prop_sub = 1 - learner_prop_basic 
teacher_prop_sub = 1 - teacher_prop_basic

ex6 = matrix(c(teacher_prop_sub, learner_prop_sub,
                teacher_prop_basic, learner_prop_basic), ncol = 2, byrow = T)
colnames(ex6)<-c("teacher", "learner")
rownames(ex6)<-c("sub", "basic")
ex6 = ex6 * 100

# get E4 and E5 data in tabular form
d.goodStim.turk = rbind(t3.f, t4.f)[,c("w.ans.cat", "Answer.sample")]
e4_and_5 = table(d.goodStim.turk)
```
...
for (n in 5:500){
  ...
  for (j in 1:50){
      ...
      # merge into single matrix for CMH test
      both_ex_tables = list(e4_and_5, sampled_ex6)
      d = do.call(cbind, both_ex_tables)
      dim = c(dim(both_ex_tables[[1]]), length(both_ex_tables))
      tab.for.power = array(d, dim)
      
      # p-value from Cochran–Mantel–Haenszel test
      ps[j] = mantelhaen.test(tab.for.power)$p.value
      #ps[j] = woolf_test(tab.for.power)$p.value
  }
