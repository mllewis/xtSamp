...
N =  <b> `r N_adults` </b> adults and  <b> `r N_child`</b> children in this study.
...
```
Some participants were in another, not analyzed condition ("noLabelParticipants"). The total number of participants before any relevant exclusions is N = <b> `r total_n`</b>. After preliminary exclusions, the total number of participants is N =  <b> `r dim(t1.f)[1]`</b>.
...
```
The number of particpants whose generalization strategy could not be catgorized was<b>  `r  dim(others.t1)[1]`</b>. The final sample was N =  <b> `r  length(t1.f$Answer.sample)`</b>.
...
```
The total before any exclusions is N = <b>  `r  dim(t2)[1]`</b>.
...
```
The total after preliminary exclusions is N = <b>  `r dim(t2.f)[1]`</b>.
...
```
The number of particpants whose generalization strategy could not be catgorized was<b>  `r  dim(others.t2)[1]`</b>. The final sample was N = <b>  `r  length(t2.f$Answer.sample)`</b>.
...
```
The total before any exclusions is N = <b>  `r  dim(l1)[1]`</b>.
...
```
The total after preliminary exclusions is N = <b> `r dim(l1.f)[1]`</b>.
...
```
The number of particpants whose generalization strategy could not be catgorized was <b> `r  dim(others.l1)[1]`</b>. The final sample was N = <b>  `r  length(l1.f$Answer.sample)`</b>.
...
```
The total before any exclusions is N = <b>  `r dim(t3)[1]`</b>.
...
```
The total after preliminary exclusions is N = <b>  `r dim(t3.f)[1]`</b>.
...
```
The number of particpants whose generalization strategy could not be catgorized was <b> `r  dim(others.t3)[1]`</b>. The final sample was N = <b>  `r  length(t3.f$Answer.sample)`</b>.
...
```
The total before any exclusions is N = <b>  `r dim(t4)[1]`</b>.
...
```
The total after preliminary exclusions is N = <b>   `r dim(t4.f)[1]`</b>.
...
The number of particpants whose generalization strategy could not be catgorized was<b>  `r  dim(others.t4)[1]`</b>. The final sample was N = <b>  `r length(t4.f$Answer.sample)`</b>.
...
#### Experiments with "good" stimuli only 
...
#### Person effect -- Does online vs. in lab matter?
```{r person_effect}
# Welch's t-test
...
#### Person effect power -- How many additional participants would we need to detect this difference?
```{r person_effect_power}
...
#### Stimuli effect -- Does the stimuli type mattter?
```{r stim_effect}
mod = predict(rma(d, var.d, data = all.es[3:4,], method = "FE"))
d1 = mod$pred  
n1 = (all.es[all.es$exp == "Exp. 1", "N.total"] +
