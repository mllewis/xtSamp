...
```{r}
criterion = "strict"  # category criterion
...
Read in data and pre-process
```{r, echo = F}
d3 <- read.csv("../data/turk_replication_3.csv")
...
for (k in 1:length(levels(d3$w.ans.cat))){
    M3 = rbind(M3, ddply(d3, .(Answer.sample), 
                     function (d) {getPs(d$w.ans.cat, levels(d$w.ans.cat)[k])}))
}
M3$exp = "Turk #3"
```
...
```{r, echo=F}
# plot
ggplot(data=M4[M4$dv == "basic" | M4$dv == "sub",], aes(x=factor(dv), y=p, fill=sample)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_linerange(aes(ymin=ciwl,ymax=ciul), position=position_dodge(.9)) +
  ylim(0,1) +
  ylab("Prop. participants showing \ngeneralization pattern") +
...
for (k in 1:length(levels(learner_xt_data))){
  M5 = rbind(M5, ddply(xt_data, .(sample), 
...
names(M2)[1] = "sample"
names(M3)[1] = "sample"
allM = rbind(M, M2, M3, M4, M5)

allM$exp <- ordered(as.factor(allM$exp), levels = c( "X&T (2007b)", "Turk #1" ,"Turk #2","Turk #3", "In Lab"))
...
e2a = d.fc(d3$w.ans.cat[d3$w.ans.cat != "other"], d3$Answer.sample[d3$w.ans.cat != "other"])
e3= d.fc(l1$ans.cat[l1$ans.cat != "other"], l1$sample[l1$ans.cat != "other"])
e4 = d.fc(xt_data$response, xt_data$sample)
```

```{r, echo = F}
es = rbind(e1, e2, e2a, e3, e4)
es$exp = c(  "Turk #1" ,"Turk #2", "Turk #3", "In Lab","X&T (2007b)")
es$exp<- ordered(as.factor(es$exp), levels = c( "X&T (2007b)", "Turk #1" ,"Turk #2", "Turk #3", "In Lab"  ))
