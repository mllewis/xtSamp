...
%Across all seven studies, the estimated effect size was .53 ($[.28,\ .78]$; Fig. 4). This estimate was only slightly reduced when the original experiment  with children was excluded  ($d = .50\ [.24,\ .75]$). Thus, we  replicate the original effect of sampling on generalization.($d = 1.49\ [0.01,\ 2.97]$), but with an effect size approximately one-third as large as the original.

\subsection{General Discussion}

Across five replication attempts, we replicate the original result in Experiments 4 and 5. 
To obtain an estimate of the overall effect size, we used meta-analytic methods to aggregate across estimates from each study. We defined a random effects model  based on the estimates from all seven studies \cite{Viechtbauer2010}. The random effect model treats each effect size estimate as an estimate of an underlying population effect size, and weights it by the precision of the estimate (sample size). Across all seven studies, the estimated effect size was .53 ($[.28,\ .78]$; Fig. 4). This estimate was only slightly reduced when the original experiment  with children was excluded  ($d = .50\ [.24,\ .75]$). Thus, aggregating across studies, we find a significant effect, consistent with the original report.

Perhaps, however, what is most striking about our results is the difference in effect size between our estimate and the original: We find an effect size that is approximately one-third as large as the original ($d = 1.49\ [0.01,\ 2.97]$). The reason for this is difficult to interpret.
LOTS OF POSSIBLE MODERATORS
- things that might matter:
* aspects of the interaction -- feel real?
* features of the teacher (reliable?)
* incentive stricutre
* variability visually
* spatial structure

Another notable difference between our sample and the original is the rate of basic generalizations. In our online studies, we saw much less generalization to the basic level than in the original studies and our in-person experiment. In fact,  our in-person experiment was the only experiment to replicate the qualitative pattern of proportions of the original studies: more basic level generalizations in the learner condition relative to the teacher condition. The reason for this difference is unclear, but one possibility is that the online medium created an incentive structure that differed from typical communicative interaction. In particular, participants in the online experiment may have placed greater value on ``getting it exactly right," and may have therefore been less likely to choose the more conservative strategy of generalizing to the basic level under uncertainty, relative to participants in the in-person experiments.
